...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	map<string,string>::iterator it = myAccounts.find(oldUserName); // finds the position of the iterator in the map
	if( it != myAccounts.end() ) {
		string password = it->second;
		try{
			myAccounts.erase(oldUserName);
			createNewAccount(newUserName, password);
		}catch(Exception& e){ throw e; }
	}else{ throw Exception( "deleteAccount()","myAccounts does not contain " + oldUserName ); }
}
...
void AccountManager::deleteAccount(const string& userName) {
	map<string,string>::iterator it = myAccounts.find(userName); // finds the position of the iterator in the map
	if( it == myAccounts.end() ) throw Exception("deleteAccount()","myAccounts does not contain " + userName);
	myAccounts.erase(it);
}
...
void AccountManager::changePassword(const string& userName,
	...
	}else throw Exception("changePassword()", "username " + userName + " does not exist.");
}
