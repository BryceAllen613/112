...
void ListTester::testInsertAfter(){
	...
	l0.append("bob");
	l0.append("ann");
	bool l0worked = l0.insertAfter("ann", "n00b");

	assert(l0worked);
	assert(l0.mySize == 3);
	assert(l0.myFirst->myItem == "bob");
	assert(l0.myFirst->myNext->myItem == "ann");
	assert(l0.myFirst->myNext->myNext->myItem == "n00b");
	assert(l0.myLast->myItem == "n00b");

	cout << " 0 " << flush;

	List<string> l1(l0);
	l1.append("phil");
	bool l1worked = l1.insertAfter("ann", "n00b2");

	assert(l1worked);
	assert(l1.mySize == 5);
	assert(l1.myFirst->myItem == "bob");
	assert(l1.myFirst->myNext->myItem == "ann");
	assert(l1.myFirst->myNext->myNext->myItem == "n00b2");
	assert(l1.myFirst->myNext->myNext->myNext->myItem == "n00b");
	assert(l1.myFirst->myNext->myNext->myNext->myNext->myItem == "phil");
	assert(l1.myLast->myItem == "phil");
...
void ListTester::testInsertBefore(){
	...
	List<string> l;
	l.append("bob");
	bool lworked = l.insertBefore("bob", "Bryce");

	assert( lworked );
	assert( l.mySize == 2 );
	assert( l.myFirst->myItem == "Bryce" );
	assert( l.myFirst->myNext->myItem == "bob" );
	assert( l.myLast->myItem == "bob" );
	...
	l0.append("bob");
	l0.append("ann");
	bool l0worked = l0.insertBefore("ann", "n00b");

	assert(l0worked);
	assert(l0.mySize == 3);
	assert(l0.myFirst->myItem == "bob");
	assert(l0.myFirst->myNext->myItem == "n00b");
	assert(l0.myFirst->myNext->myNext->myItem == "ann");
	assert(l0.myLast->myItem == "ann");

	cout << " 0 " << flush;

	List<string> l1(l0); //l0 = bob, n00b, ann
	bool l1worked = l1.insertBefore("bob", "n00b2");
	//l1 = n00b2, bob, n00b, ann

	assert(l1worked);
	assert(l1.myFirst->myItem == "n00b2");
	assert(l1.myFirst->myNext->myItem == "bob");
	assert(l1.myFirst->myNext->myNext->myItem == "n00b");
	assert(l1.myFirst->myNext->myNext->myNext->myItem == "ann");
	assert(l1.myLast->myItem == "ann");
...
void ListTester::testInsertionOperator(){
	...
	ofstream fout("listTest3out.txt");
	List<string> myLine;
	myLine.append("Bryce");
	myLine.append("Matt");
	myLine.append("Ricky");
	fout << myLine << flush;
	ifstream fin("listTest3out.txt");
	string stringLine;
	fin >> stringLine;
	assert( stringLine == "Bryce=>Matt=>Ricky=>" );
	cout << " 0 " << flush;
